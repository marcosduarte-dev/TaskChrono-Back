...
func (p *Timer) FindMyTimers(page, limit int, sort string) ([]*entity.Timer, error) {
	...
	if page != 0 && limit != 0 {
		...
		err = p.DB.Order("task_id " + sort).Preload("Task").Preload("Task.Project").Find(&timer).Error
	}
	return timer, err
}
...
func (p *Timer) FindByTaskID(taskID string) ([]*entity.Timer, error) {
	...
	err := p.DB.Preload("Task").Preload("Task.Project").Where("task_id = ?", taskID).Find(&timer).Error
